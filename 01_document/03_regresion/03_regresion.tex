...
\noindent dónde $F_Y(\cdot|\cdot,\cdot,\cdot)$ es la función de distribución acumulada de la distribución Weibull reparametrizada dada en \ref{sec2.2}.Entonces la función de verosimilitud de las variables observadas $Z_{1},Z_{2},\dots,Z_{n}$ es dada por lo siguiente:
...
Luego, la función de log-verosímilitud es dada por:

\[l(\boldsymbol{\theta})=\sum_{i=1}^{n} \log \left( F(u_{i}|q_{t_{i}},\alpha,t) - F\left( l_{i}|q_{t_{i}},\alpha,t \right) \right)\]

\[ l(\boldsymbol{\theta})= \sum_{i=1}^{n} log\left( \left( 1-t \right)^{(\frac{u_i}{\exp(x_{i}^{T}\beta)})^{\alpha}}} - \left( 1-t \right)^{(\frac{l_{i}}{\exp(x_{i}^{T}\beta)})^{\alpha}} \right) \]
...
\[ \frac{\partial l}{\partial \alpha}= \sum_{i=1}^{n} \frac{1}{(1-t)^{\phi_{u}}-(1-t)^{\phi_{l}}} log(1-t)(\phi_u \log(u_i*\gamma_i)(1-t)^{\phi_u} - \phi_l \log(l_i*\gamma_i)(1-t)^{\phi_{l}})\]

\[\frac{\partial l}{\partial \beta_{j}}=\sum_{i=1}^{n} \frac{\alpha x_j \log(1-t)}{(1-t)^{\phi_u} - (1-t)^{\phi_l}} (\phi_l(1-t)^{\phi_l}-\phi_u(1-t)^{\phi_u}) \]

\noindent en dónde:
\[ \gamma_{i} = \exp(-\eta_{i})\]
\[ \eta_{i} = x_{i}^{T}\boldsymbol{\beta}\]
\[ \phi_u = \left( \frac{u_i}{e^{x_i^{T}\beta}} \right)^{\alpha}\]
\[ \phi_l = \left( \frac{l_i}{e^{x_i^{T}\beta}} \right)^{\alpha}\]

Dichos estimadores de máxima verosimilitud son consistentes  (es decir, que $\boldsymbol{\hat{\theta}} \rightarrow \boldsymbol{\theta}$ cuando $n \rightarrow \infty$) y asíntoticamente normales\footnote{Se asume que la función de densidad cumple con las condiciones de regularidad expuestas en \cite{casella:berg}} con distribución:
...
\noindent dónde $\theta$ es el verdadero valor del parámetro, $\hat{\theta}_{j}$ la estimación obtenida en la $j$-ésima réplica, M es el número de réplicas, y $IC_j$ es el intervalo de confianza al 95\% obtenido en la $j$-ésima réplica.
