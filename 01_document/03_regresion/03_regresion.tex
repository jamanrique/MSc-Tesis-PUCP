...
\noindent cuando $n \rightarrow \infty$. $\mathcal{I}(\theta)$ es la matriz de información de Fisher, la cual en nuestro modelo tiene la estructura:
...
Los errores estándares de cada coeficiente se estiman a través de la matriz de información de Fisher observada, $\mathcal{I}(\hat{\theta})$, la cual es la matriz evaluada en los estimadores de máxima verosimilitud \cite{casella:berg}. Los errores estándares corresponden a la raíz cuadrada de cada elemento de la diagonal. Finalmente, en el marco de la inferencia clásica, los intervalos de confianza para cada parámetro está definido de la forma:

\[\hat{\theta} \pm \frac{z_{1-\frac{\alpha}{2}}}{\sqrt{\mathcal{I}(\hat{\theta)}}}.\]

\noindent en dónde $\mathcal{I}(\hat{\theta})$ corresponde a la matriz de información de Fisher observada.
...
\caption{Evaluación del estudio de simulación}
\end{longtable}
\end{landscape}
